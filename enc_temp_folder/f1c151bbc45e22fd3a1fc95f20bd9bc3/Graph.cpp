...
void Graph::OnPaint()
{
	...
	{
		dc.MoveTo(0, 2*rc.CenterPoint().y-1);
		dc.LineTo(rc.right, 2*rc.CenterPoint().y-1);
		dc.MoveTo(0, 0);
		dc.LineTo(0, rc.bottom);
	}
	...
	if (n)
	{
		...
		else
		{
			for (int i = 1; i <= n; i++)
			{
				if ((i % 100) == 0)
				{
					dc.MoveTo(Points[i-1].x, 180 - 5);
					dc.LineTo(Points[i-1].x, 180 + 5);
					CString number;
					number.Format(L"%d", i);
					dc.TextOutW(Points[i-1].x - 30, 150, number);
				}
			}
			dc.SelectObject(penG);
			for (int i = 0; i < n; i++)
